...
-(void) _setGrabberEnabled:(BOOL)arg1 {
	if(arg1 == YES) {
		...
		UIView* batteryBarFull = [[UIView alloc]initWithFrame:CGRectMake(1, 0.5, (batteryBar.frame.size.width-1) * [[%c(SBUIController) sharedInstance]batteryCapacity], 1)];

		batteryBarFull.backgroundColor = [[%c(SBUIController) sharedInstance]batteryCapacityAsPercentage] > 20 ? 							//If above 20%
										([[%c(SBUIController) sharedInstance]isOnAC] ? [UIColor greenColor] : [UIColor whiteColor]) :		//If charging, color is green, otherwise white
										[UIColor redColor];
		
		[batteryBar addSubview:batteryBarFull];
		[batteryBarFull release];
		[formatDate release];
